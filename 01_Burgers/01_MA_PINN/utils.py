...
def plot_gamma_curve(
    epoch, 
    epoch_log, gamma_ic_log, gamma_bc_log
):
    plt.figure(figsize=(8, 4))
    plt.plot(epoch_log, gamma_ic_log, alpha=.7, label="gamma_ic", c="tab:orange")
    plt.plot(epoch_log, gamma_bc_log, alpha=.7, label="gamma_bc", c="tab:green")
    plt.legend(loc="upper right")
    plt.grid(alpha=.5)
    plt.tight_layout()
...
def plot_grad_dist(
    ...
    sns.histplot(gamma_ic * grad_dist_ic,  stat="density", element="step", fill=False, alpha=.7, label="grad_ic",  color="tab:orange")
    sns.histplot(gamma_bc * grad_dist_bc,  stat="density", element="step", fill=False, alpha=.7, label="grad_bc",  color="tab:green")
    plt.xlabel("gradient")
    plt.ylabel("density")
    plt.xscale("linear")
    plt.yscale("symlog")
