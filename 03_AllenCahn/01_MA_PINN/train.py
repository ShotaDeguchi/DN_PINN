...
def main(args):
    ...
    # define a domain
    data = io.loadmat("../reference_AllenCahn.mat")   # taken from Raissi+2019
    ...
    # sample training points (Wight+2021)
    N_pde = int(2e4)
    N_ic  = int(512)
    N_bc  = int(1e2)
    ...
    for epoch in range(1, args.epochs+1):
        # update gamma
        if epoch % tau == 0:
            _grad_pde, _grad_ic, _grad_bc1, _grad_bc2 = model.update_gamma(
                t_pde, x_pde, g_pde, 
                t_ic,  x_ic,  u_ic, 
                t_bc1, x_bc1, u_bc1, 
                t_bc2, x_bc2, u_bc2
        ...
        if args.batch_size == -1:
            loss_glb, loss_pde, loss_ic, loss_bc1, loss_bc2 = model.train_fb(
                t_pde, x_pde, g_pde, 
                t_ic,  x_ic,  u_ic, 
                t_bc1, x_bc1, u_bc1, 
                t_bc2, x_bc2, u_bc2
        ...
        # mini-batch training
            ...
            loss_glb, loss_pde, loss_ic, loss_bc1, loss_bc2 = model.train_mb(
                args.batch_size, 
                t_pde, x_pde, g_pde, 
                t_ic,  x_ic,  u_ic, 
                t_bc1, x_bc1, u_bc1, 
        ...
        loss_bc1_log.append(loss_bc1)
        loss_bc2_log.append(loss_bc2)

        # log gamma
        gamma_ic = model.gamma_ic.numpy()
        gamma_bc1 = model.gamma_bc1.numpy()
        gamma_bc2 = model.gamma_bc2.numpy()
        gamma_ic_log.append(gamma_ic)
        gamma_bc1_log.append(gamma_bc1)
        gamma_bc2_log.append(gamma_bc2)
        ...
        g_mse = np.mean(np.square(g_))
        g_sem = np.std (np.square(g_), ddof=1) / np.sqrt(float(g_.shape[0]))
        ...
        logger_data = \
            ...
            f"loss_bc1: {loss_bc1:.3e}, " \
            f"loss_bc2: {loss_bc2:.3e}, " \
            f"u_l2: {u_l2:.3e}, " \
            f"g_l2: {g_l2:.3e}, " \
            f"gamma_ic: {gamma_ic:.3e}, " \
            f"gamma_bc1: {gamma_bc1:.3e}, " \
            f"gamma_bc2: {gamma_bc2:.3e}, " \
        ...
        if epoch % 1000 == 0:
            ...
            plot_gamma_curve(
                epoch, 
                epoch_log, gamma_ic_log, gamma_bc1_log, gamma_bc2_log
            )
            plot_grad_dist(
                epoch,
                model, 
