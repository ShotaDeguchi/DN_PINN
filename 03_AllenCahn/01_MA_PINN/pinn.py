...
Gradient Pathologies of PINN (Wang+2021)
********************************************************************************
"""
...
class PINN(tf.keras.Model):
    ...
    def update_gamma(
        ...
        """
        Updates gamma's (weights associated with loss components) accoding to Wang+2021.
        """
        ...
        # stats
        _grad_pde = tf.math.reduce_max (tf.abs(_grad_pde), axis=None)
        _grad_ic  = tf.math.reduce_mean(tf.abs(_grad_ic),  axis=None)
        _grad_bc1 = tf.math.reduce_mean(tf.abs(_grad_bc1), axis=None)
        _grad_bc2 = tf.math.reduce_mean(tf.abs(_grad_bc2), axis=None)

        # max / mean
        gamma_ic  = _grad_pde / (self.gamma_ic  * _grad_ic)
        gamma_bc1 = _grad_pde / (self.gamma_bc1 * _grad_bc1)
        gamma_bc2 = _grad_pde / (self.gamma_bc2 * _grad_bc2)
        ...
        return _grad_pde, _grad_ic, _grad_bc1, _grad_bc2
